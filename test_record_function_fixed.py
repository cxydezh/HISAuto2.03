#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
测试修复后的录制功能
"""

import tkinter as tk
from tkinter import ttk, messagebox
import sys
import os

# 添加项目根目录到路径
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

from utils.action_recorder_fixed import ActionRecorder
from config.config_manager import ConfigManager

class MockHomeTab:
    """模拟HomeTab类"""
    
    def __init__(self):
        self.my_window = tk.Tk()
        self.my_window.title("测试主窗口")
        self.my_window.geometry("600x400")
        self.action_group_id = 1  # 模拟选中的行为组ID
        
        # 创建测试界面
        self._create_test_ui()
        
    def _create_test_ui(self):
        """创建测试界面"""
        # 主框架
        main_frame = ttk.Frame(self.my_window, padding="20")
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # 标题
        title_label = ttk.Label(main_frame, text="录制功能测试", font=("Arial", 16, "bold"))
        title_label.pack(pady=(0, 20))
        
        # 说明
        info_label = ttk.Label(main_frame, text="点击下方按钮测试录制功能", font=("Arial", 12))
        info_label.pack(pady=(0, 20))
        
        # 测试按钮
        test_button = ttk.Button(main_frame, text="开始录制测试", command=self._test_recording)
        test_button.pack(pady=10)
        
        # 配置信息显示
        self._show_config_info(main_frame)
        
    def _show_config_info(self, parent):
        """显示配置信息"""
        try:
            config = ConfigManager()
            end_record_key = config.get_value('Shortcuts', 'endrecord')
            
            config_frame = ttk.LabelFrame(parent, text="配置信息", padding="10")
            config_frame.pack(fill=tk.X, pady=10)
            
            ttk.Label(config_frame, text=f"结束录制快捷键: {end_record_key or '未配置'}", 
                     font=("Arial", 10)).pack(anchor=tk.W)
            
        except Exception as e:
            ttk.Label(parent, text=f"获取配置失败: {str(e)}", 
                     font=("Arial", 10), foreground="red").pack(anchor=tk.W)
    
    def _test_recording(self):
        """测试录制功能"""
        try:
            # 创建录制管理器
            recorder = ActionRecorder(self)
            
            # 显示录制选项
            result = recorder.show_record_options()
            
            if result:
                print("录制选项窗口显示成功")
            else:
                print("录制选项窗口显示失败")
                
        except Exception as e:
            messagebox.showerror("错误", f"测试录制功能失败: {str(e)}")
            print(f"错误详情: {e}")
            import traceback
            traceback.print_exc()
    
    def _refresh_action_list(self):
        """模拟刷新行为列表"""
        print("模拟刷新行为列表")
        
    def run(self):
        """运行测试"""
        self.my_window.mainloop()

def main():
    """主函数"""
    print("开始测试录制功能...")
    
    # 创建测试窗口
    home_tab = MockHomeTab()
    
    # 运行测试
    home_tab.run()

if __name__ == "__main__":
    main() 