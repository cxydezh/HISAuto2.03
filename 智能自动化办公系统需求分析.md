...
21.	ActionSuitList（Suit行为组记录表）：ID(自动生成，该表的主键)、action_Type（mouse、keyboard、codeTxt、printscreen、ai、class、picture、Action_function），ActionName、nextID（用于标记下一步所要进行的步骤ID）、debugGroupID（Debug调试用ID，用来指定要进入哪个Debug行为组，与ActionsDebugGroup中的ID相对应）、setupTime、updateTime、actionNote（行为元备注）。
说明：该表用于用户创建行为组的组套。
22.	ActionsSuitGroup（Suit行为组表）：ID(自动生成)、GroupID（与ActionSuitListGroup中的GroupID相对应，外键）、SortNum（排序编码，进行排序时的依据）、ActionListGroupName、GroupRank、ExcelName（行为组运行所涉及的Excel文件的名称，其位置详见下文的系统文件结构章节）、ExcelSheetNum（系统要执行的循环列表所在的Sheet编号，如第一个Sheet、第二个Sheet，字段类型为int）、ExcelColumn（系统要执行的循环列表中，提供给系统用来监测列表是否已经执行结束的列编号，一般每一行的该列都有内容，系统执行每一次循环需要检测该行是否为空，如为空则提示循环已到列表的末尾。该字段类型为int）、LastCircleLocal（上一次循环中，运行到了Excel表格中的第几行，该字段为该行的行数，行为组中断运行时记录用）、LastCircleNode（上一次循环中，系统运行到了哪个行为元，该字段行为元的ID号，行为组中断运行时记录用）、AboutTime（系统运行一个循环预计所需的时间，该时间为该行为组的上一个完整循环所用的时间）、setupTime、updateTime、ActionListGroupNote（行为组备注）。
...
2.	上述行为组层级表格（包括Debug行为组层级表格）和行为组套（包括Debug行为组套）中提到的grouprank（用于对该动作记录在哪个组套或行为组层级里面的标注，注意命名规则，应设置为与树形结构相关的字段内容）字段，采用最多5级的命名原则，分别使用A、B、C、D、E代表。字段格式为：A_B_C_D_E_。字段格式含义为：每一个字母后面跟一个数值，分别表示该字母所代表的分层级别或组套级别的第几项。其中A级别代表总体的分层或组套，一共分三层——“个人”、“科室”、“全局”。当A后面为3时，代表该分层或组套为全局级别的分层或组套；当A后面为“2”时，代表该分层或组套为科室级别的分层或组套；当A后面为1时，代表该分层或组套为个人级别的分层或组套。B、C、D、E级别后面的数值只需要是正值或零即可。举个例子“A3B1C1D1E1”代表该分层或组套为全局级别的分层或组套，它为B层级中编号为1的层级中C层级中编号为1的层级中的D层级中的编号为1的层级中的E层级中编码为1的层级。如果B、C、D、E级别中的数值为零，则代表该分层或组套为该级别分层或组套的根，后面的级别中的数值也必须为零。举个例子，“A2B1C0D0E0”代表该分层或组套为科室组套中的第B1级分层或组套的根，该记录的GroupName即为B层级中编号为1的层级的组名。具体可见下图：
👤 A1层级 (全局)
  ├─📁 B1层级
  │  ├─📁 C1层级
  │     ├─📁 D1层级
...
注意：
1.进行行为组创建时，尽量将AI相关功能部署在第二级别的位置，也就是AxB0级别。因为网页上只能操作这个级别的AI功能。其他级别的AI功能看不到。
...
关于树形结构的排序问题：
1.行为元的执行顺序，先根据rank级别排序，然后根据sort排序，如果next_id存在，优先执行next_id对应的行为元。
2.行为元的rank级别，仍然延续之前的ABCDE编排。

请帮忙改写_load_action_group_data函数实现以下功能：
1.可以根据要求为接受的Treeview类型的参数进行树结构的数据填充。
2.接受3个参数，分别是：workTreeview（是Treeview控件类型数据），treeDatatxt(用来描述用什么数据填充树形控件的数据，为text格式）,group_id(为int格式，表示要填充的树形控件的组ID,可以为空)。
3.treeDatatxt可以是：ActionList、ActionsGroupHierarchy、ActionSuitList、ActionsSuitGroupHierarchy、ActionDebugList、ActionsDebugGroupHierarchy。分别对应model文件中相对应的ORM模型。
4.当treeDatatxt为：ActionList、ActionSuitList、ActionDebugList时，控件workTreeview加载的数据表数据按照数据表中的list_rank字段进行分层，相同层的数据根据数据表中的action_sort_num字段进行排序；当treeDatatxt为：ActionsGroupHierarchy、ActionsSuitGroupHierarchy、ActionsDebugGroupHierarchy时，控件workTreeview加载的数据表数据按照数据表中的group_rank字段进行分层，相同层的数据根据数据表中的sort_num字段进行排序；
5.list_rank和group_rank字段的命名规则：采用最多5级的命名原则，分别使用A、B、C、D、E代表。字段格式为：A_B_C_D_E_。字段格式含义为：每一个字母后面跟一个数值，分别表示该字母所代表的分层级别或组套级别的第几项。其中A级别代表总体的分层或组套，一共分三层——“个人”、“科室”、“全局”。当A后面为3时，代表该分层或组套为全局级别的分层或组套；当A后面为“2”时，代表该分层或组套为科室级别的分层或组套；当A后面为1时，代表该分层或组套为个人级别的分层或组套。B、C、D、E级别后面的数值只需要是正值或零即可。举个例子“A3B1C1D1E1”代表该分层或组套为全局级别的分层或组套，它为B层级中编号为1的层级中C层级中编号为1的层级中的D层级中的编号为1的层级中的E层级中编码为1的层级。如果B、C、D、E级别中的数值为零，则代表该分层或组套为该级别分层或组套的根，后面的级别中的数值也必须为零。举个例子，“A2B1C0D0E0”代表该分层或组套为科室组套中的第B1级分层或组套的根，该记录的GroupName即为B层级中编号为1的层级的组名。具体可见下图：
👤 A1层级 (全局)
  ├─📁 B1层级
  │  ├─📁 C1层级
  │     ├─📁 D1层级
...
🌍 A3层级 (个人)
  └─⭕ B0层级 (根节点)
6.当treeDatatxt为：ActionList、ActionSuitList、ActionDebugList时，workTreeview中加载的数据为从对应的数据表中筛选出的字段group_id等于参数group_id的数据，添加节点iid为经过parse_group_rank函数转化后的list_rank字段值。如果对应的数据表的action_type字段的值为“group",则添加节点的text_name为“📁”，value值为action_name字段值、action_note字段值、id字段值。如果对应的数据表的action_type字段的值不为“group",则添加节点的text_name为“📄”，value值为action_name字段值、action_note字段值、id字段值。
7.当treeDatatxt为：ActionsGroupHierarchy、ActionsSuitGroupHierarchy、ActionsDebugGroupHierarchy时，workTreeview中加载的数据分3类：1类为从对应的数据表中筛选出的字段group_rank字段值起始字符串为“A1B”且user_id等于全局参数globalvariable.USER_ID 的数据；2类为从对应的数据表中筛选出的字段group_rank字段值起始字符串为“A2B”且department_id等于全局参数globalvariable.USER_DEPARTMENT_ID 的数据；3类为从对应的数据表中筛选出的字段group_rank字段值起始字符串为“A3B”的数据。添加节点iid为经过parse_group_rank函数转化后的list_rank字段值。如果对应的数据表的action_type字段的值为“group",则添加节点的text_name为“📁”，value值为action_name字段值、action_note字段值、id字段值。如果对应的数据表的action_type字段的值不为“group",则添加节点的text_name为“📄”，value值为action_name字段值、action_note字段值、id字段值。